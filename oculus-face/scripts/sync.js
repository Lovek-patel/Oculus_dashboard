// Pulls today + tomorrow's Google Calendar events and open Google Tasks,
// writes them to data.json at the repo root. Run by .github/workflows/sync.yml
// on a schedule, or locally with the same env vars for testing.
...
async function fetchEvents() {
  const timeMin = startOfDay(0).toISOString();
  const timeMax = startOfDay(2).toISOString(); // today + tomorrow
...
async function fetchTasks() {
  ...
  // Keep incomplete tasks, plus anything completed today so it doesn't vanish instantly.
